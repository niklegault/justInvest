...
echo "--- 2. Downloading Dependencies into ${LIB_DIR} using wget ---"
...
download_file() {
    ...
    if wget -O "${output_path}" -c --tries=5 "${url}"; then
        echo "${filename} downloaded successfully."
    else
        echo "ERROR: Failed to download ${filename}. This usually indicates a network issue or an invalid URL. Exiting."
        exit 1
    fi
}

download_file "${SPRING_URL}" "${LIB_DIR}/${SPRING_FILE}" "${SPRING_FILE}"

download_file "${LOGGING_URL}" "${LIB_DIR}/${LOGGING_FILE}" "${LOGGING_FILE}"
